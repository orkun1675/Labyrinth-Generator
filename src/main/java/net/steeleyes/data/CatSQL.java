...
public class CatSQL {
            ...
            while (rs.next()) {
                ...
                if (world == null) {
                    ...
                    while (rs2.next()) {
                        ...
                        if (roof == 0) roof = cube.guessRoofSize();
                        if (room == 0) room = cube.guessRoomSize();
    ...
    private int guessFloorDepth(Block blk) {
        int floor = 3;
        while (true) {
            if (blk.getType() == Material.CHEST || blk.getType() == Material.TRAP_DOOR) return floor;
            floor++;
            blk = blk.getRelative(BlockFace.UP);
            if (blk == null || floor > 20) return 3;
        }
    }
}  
